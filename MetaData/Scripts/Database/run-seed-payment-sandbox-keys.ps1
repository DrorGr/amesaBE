...
<#
.SYNOPSIS
  Applies seed-payment-sandbox-translation-keys.sql via Content DB connection from SSM.

.EXAMPLE
  .\run-seed-payment-sandbox-keys.ps1
  .\run-seed-payment-sandbox-keys.ps1 -Apply -Region eu-north-1
#>
param(
    [switch]$Apply,
...
$sqlFile = Join-Path (Get-RepoRoot) "MetaData\Scripts\Database\seed-payment-sandbox-translation-keys.sql"
...
Write-Host "Done. Flush Redis translation cache if Content caches GET /translations."
