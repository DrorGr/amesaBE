        ...
        if (!string.IsNullOrWhiteSpace(metaAppId) && !string.IsNullOrWhiteSpace(metaAppSecret))
        {
            authBuilder.AddFacebook(options =>
            {
                ...
                options.Events.OnTicketReceived = context =>
                {
                    ...
                    var redirectUri = context.Properties?.RedirectUri ?? "NULL";
                    var hasTempToken = context.Properties?.Items?.ContainsKey("temp_token") ?? false;
                    logger?.LogInformation("[DEBUG] OnTicketReceived (Meta): redirectUri={RedirectUri} hasTempToken={HasTempToken}", redirectUri, hasTempToken);
                    return Task.CompletedTask;
                };
            ...
            !string.IsNullOrWhiteSpace(applePrivateKey))
        {
            authBuilder.AddApple(options =>
            {
                ...
                options.Events.OnTicketReceived = context =>
                {
                    ...
                    var redirectUri = context.Properties?.RedirectUri ?? "NULL";
                    var hasTempToken = context.Properties?.Items?.ContainsKey("temp_token") ?? false;
                    logger?.LogInformation("[DEBUG] OnTicketReceived (Apple): redirectUri={RedirectUri} hasTempToken={HasTempToken}", redirectUri, hasTempToken);
                    return Task.CompletedTask;
                };
